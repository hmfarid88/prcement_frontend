...
const Page = () => {
    ...
    useEffect(() => {
        if (!id) return;
        fetch(`${apiBaseUrl}/paymentApi/getEmployeepaymentInfo/${id}`)
    ...
    const handleUpdateSubmit = async (e: any) => {
        ...
        setPending(true);
        try {
            const response = await fetch(`${apiBaseUrl}/paymentApi/updateEmployeePayInfo/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ date, employeeName, year, month, note, amount }),
            });
    ...
    return (
        <div className='container-2xl min-h-screen pb-5'>
            <div className="flex flex-col w-full items-center justify-center p-2">
                ...
                <label className="form-control w-full max-w-xs pt-2">
                    <div className="label">
                        <span className="label-text-alt">EMPLOYEE</span>
                    </div>
                    <div className="flex justify-between gap-2">
                        <p className='capitalize w-[50%] p-2 bg-white text-black rounded-md'>{employeeName}</p>
                        <Select className="text-black w-[50%]" name="retailer" onChange={(selectedOption: any) => setEmployeeName(selectedOption.value)} options={employeeOption} />
                    </div>
                </label>
