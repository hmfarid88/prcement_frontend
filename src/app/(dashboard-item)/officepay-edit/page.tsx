...
const Page = () => {
    ...
    useEffect(() => {
        if (!id) return;
        fetch(`${apiBaseUrl}/paymentApi/getOfficepaymentInfo/${id}`)
    ...
    const handleUpdateSubmit = async (e: any) => {
        e.preventDefault();
        if (!id || !date || !paymentName || !paymentNote || !amount) {
            toast.warning("Item is empty !")
            return;
        }
        setPending(true);
        try {
            const response = await fetch(`${apiBaseUrl}/paymentApi/updateOfficepayInfo/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ id, date, paymentName, paymentNote, amount }),
            });
 ...
 const handleDeleteSubmit = async (e: any) => {
        e.preventDefault();
        try {
            const response = await fetch(`${apiBaseUrl}/api/deleteOfficePayById/${id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        <div className='container-2xl min-h-screen pb-5'>
            <div className="flex flex-col w-full items-center justify-center p-2">
                ...
                <label className="form-control w-full max-w-xs pt-2">
                    ...
                    <input type='text' className='input input-md h-[40px] bg-white text-black border rounded-md border-slate-300' value={paymentName} onChange={(e: any) => setPaymentName(e.target.value)} placeholder='Type Here' />
                </label>
                <label className="form-control w-full max-w-xs pt-2">
                    <div className="label">
                        <span className="label-text-alt">NOTE</span>
                    </div>
                    <input type='text' className='input input-md h-[40px] bg-white text-black border rounded-md border-slate-300' value={paymentNote} onChange={(e: any) => setPaymentNote(e.target.value)} placeholder='Type Here' />
                </label>
                <label className="form-control w-full max-w-xs pt-2">
                    <div className="label">
