...
const Page = () => {
    ...
    useEffect(() => {
        if (!id) return;
        fetch(`${apiBaseUrl}/retailer/getRetailerPayment?id=${id}`)
    ...
    const handleUpdateSubmit = async (e: any) => {
        e.preventDefault();
        if (!date || !retailerName || !note || !amount) {
            toast.warning("Item is empty !")
            return;
        }
        setPending(true);
        try {
            const response = await fetch(`${apiBaseUrl}/retailer/updateRetailerPayInfo/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ date, retailerName, note, amount }),
            });
