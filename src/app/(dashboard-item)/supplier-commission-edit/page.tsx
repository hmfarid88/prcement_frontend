...
const Page = () => {
    ...
    useEffect(() => {
        if (!id) return;
        fetch(`${apiBaseUrl}/paymentApi/getSupplierCommissionInfo/${id}`)
    ...
    const handleUpdateSubmit = async (e: any) => {
        ...
        setPending(true);
        try {
            const response = await fetch(`${apiBaseUrl}/paymentApi/updateSupplierCommissionInfo/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ date, supplierName, year, month, note, amount }),
            });
