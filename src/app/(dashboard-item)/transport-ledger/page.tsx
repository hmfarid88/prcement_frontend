...
const Page = () => {
  ...
  const handleDetails = (transport: string) => {
    if (!transport) {
      toast.warning("Transport name is empty!");
      return;
    }
    router.push(`/details-transport?transport=${encodeURIComponent(transport)}&username=${encodeURIComponent(username)}`);

  }

  useEffect(() => {
    fetch(`${apiBaseUrl}/supplierBalance/transport/balance?username=${encodeURIComponent(username)}`)
      ...
      <div className="flex flex-col w-full min-h-[calc(100vh-228px)] p-4 items-center justify-center">
        <div className="flex w-full justify-between pl-5 pr-5 pt-1">
          ...
          <div className="flex gap-2">
            <ExcelExport tableRef={contentToPrint} fileName="transport_ledger" />
                ...
                <tbody>
                  {filteredProducts?.map((product, index) => (
                    <tr key={index}>
                      ...
                      <td><button onClick={() => handleDetails(product?.transport)} className="btn btn-xs btn-info">Details</button></td>
