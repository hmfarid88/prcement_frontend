...
const RetailerCommission = () => {
    ...
    const handleSupplierPayment = async (e: any) => {
        e.preventDefault();
        if (!retailerName || !month || !year || !amount) {
            toast.warning("Item is empty !");
            return;
        }
        setPending(true);
        try {
            const response = await fetch(`${apiBaseUrl}/paymentApi/retailerCommission`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ date, retailerName, month, year, amount, note, username }),
            });
    ...
    return (
        <div className='flex flex-col gap-2 items-center justify-center p-2'>
                ...
                <label className="form-control w-full max-w-xs">
                    ...
                    <select className='select select-bordered text-black bg-white' onChange={(e: any) => { setMonth(e.target.value) }}>
                        <option selected disabled>Select . . .</option>
